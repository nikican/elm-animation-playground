...
initialModel =
    { style =
        Animation.style
            ...
            , Animation.path signal1965
            ]
    , index = 1
    }


signal1965 : BatSignal
signal1965 =
...
update action model =
    case action of
        Morph ->
            let
                wrappedIndex =
                    if List.length batSignals < model.index then
                        model.index - List.length batSignals
                    else
                        model.index

                newPath =
                    Maybe.withDefault signal1966 <|
                        List.head <|
                            (List.drop wrappedIndex batSignals)
                                ++ (List.take wrappedIndex batSignals)
            in
                { model
